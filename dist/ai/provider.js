...
function extractComparisonCandidates(sourceText) {
    const candidates = [];
...
function findMissingCoverageByInventory(bundle, inventory) {
...
function toKeyTermObject(value) {
...
function toInsightObject(value) {
    if (typeof value === "object" && value !== null) {
        const candidate = value;
        return {
...
function toStringArray(values, minLength, fallbackPrefix) {
...
function normalizeChallengeObject(value, index) {
    const source = typeof value === "object" && value !== null ? value : {};
    ...
    const starterCode = String(source.starterCode ??
        source.codeTemplate ??
        source.template ??
    ...
    const testCases = rawTestCases
        .map((testCase) => {
        const t = typeof testCase === "object" && testCase !== null ? testCase : {};
        return {
...
export function normalizeLessonBundleShape(input) {
    ...
    if (Array.isArray(payload.challenges)) {
        ...
        normalized.challenges = challenges.map((challenge, index) => normalizeChallengeObject(challenge, index));
    }
    return normalized;
}
...
function buildCoverageChecklist(sourceText) {
    const checks = [
