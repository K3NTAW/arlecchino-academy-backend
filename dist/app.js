...
export function createApp(aiProvider, db) {
    ...
    app.post("/api/generate", async (req, res, next) => {
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("AI request timeout")), env.REQUEST_TIMEOUT_MS));
            ...
            const returnChallenges = stored?.challenges.map((challenge) => ({
                ...challenge.payload,
                id: String(challenge.id),
                difficulty: challenge.difficulty
            })) ?? parsed.challenges;
