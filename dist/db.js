...
export class DatabaseService {
    ...
    async findLessonByHashAndLanguage(hash, language) {
    ...
    `);
        ...
        return {
            ...
            streakDays,
            level,
            recentLesson: recentLessonRes.rowCount && recentLessonRes.rowCount > 0
                ? {
    ...
    `);
        const badges = badgesRes.rowCount && badgesRes.rowCount > 0
    ...
    async getChallengeById(challengeId) {
        const res = await this.pool.query(`SELECT id, lesson_id, payload, type, difficulty FROM challenges WHERE id = $1`, [challengeId]);
        if (res.rowCount === 0) {
            return null;
        }
    ...
    async saveAttempt(challengeId, isCorrect, details) {
